...
//    Mat Map2Mat(uint8_t map[][], QPoint point);
...
//    void imgGrayEvent(uint8_t img[][], QLabel *label, uint8_t multiple);
//    void mapEvent(uint8_t map[][], QLabel *label, uint8_t multiple);
...
//    QSplineSeries *series8;
    ...
    int recv_wifiNum;
    char local_wifiBuff[WifiBuffer_AllNum];
    uint16 ac_pix_index;
    uint16 pix_index;
    uint16 pix_num;
    void Camera_FPS();
