...
void Widget::allinit(){
    ...
    get_the_serialport();
    Widget::setWindowTitle("SmartLink");
...
bool Widget::readData()//读取SD卡中的变量信息
{
    if(readType_flag == 1)//wifi_Read == 1
    {
        ...
        int recv_sdNum = 5;
        size_t bytesToCopy = my_row * _my_col * sizeof(uint8);
        memcpy(&imgGray[0], &sdBuff[recv_sdNum], bytesToCopy);

        recv_sdNum = recv_sdNum + my_row * _my_col;
        ...
        /// ////////////////////////////////////////////////变量区
        /// ////////////////////////////////////////////////变量区
        if(sdBuff[recv_sdNum++] != 'V' || sdBuff[recv_sdNum++] != 'A' || sdBuff[recv_sdNum++] != 'L' || sdBuff[recv_sdNum++] != 'U' || sdBuff[recv_sdNum++] != 'E')
        {
            image_trans_Flag = false;
            return false;
        }

        fps = sdBuff[recv_sdNum];//不要改变//误删//误删这是上位机FPS显示的暂存值
    ...
    else if(readType_flag == 2)//serial_Read == 2
    {
        ...
        for(uint16_t i = 0; i < SerialBuffer_AllNum; ++i)
        {
            sdBuff[i] = FKTMDfileData.at(offset + i);  //char sdBuff[512];
        }
        if(sdBuff[0] != 'C' || sdBuff[1] != 'Z' || sdBuff[2] != 'Y' || sdBuff[3] != 'N' || sdBuff[4] != 'B')
...
bool Widget::readgotoData()
{
    ...
    else if(readType_flag == 2)//serial_Read == 2
    {
        ...
        if(data_goto>1&&data_goto<(int)dataNum)
       {
            offset += (data_goto-dataCnt)*SerialBuffer_AllNum;
            dataCnt=data_goto;
        }
        ui->Pic_verticalSlider->setValue(dataCnt);
        if(readData())
...
{
        Mat Graymat = GrayMap2Mat(imgGray, point);
        showMat(Graymat, ui->imgGray_lable);

        Mat mat = Map2Mat(_Exiacn, point);
        showMat(mat, ui->Erzhihua_lable);
}
...
Mat Widget::Map2Mat(uint8_t map[my_row][my_col], QPoint point)
{  //MUTIPLE应该是图像放大倍数，CV_8UC3表示图像是3通道的
    Mat mat(my_row * MULTIPLE, my_col * MULTIPLE, CV_8UC3);
    ...
    {  //MUTIPLE是放大倍数，/MUTIPLE当然是真实的坐标了，不过是CV坐标系的，转成代码的坐标系还需要坐标转换一下
        ...
        for (int i = 0; i < mat.cols; ++i)
        {
            ...
            //BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR
            //BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR//BGR
            uint8_t nowPixel = map[(j / MULTIPLE)][i / MULTIPLE];
            if(nowPixel == 255)
            {
                ptr[i][0] = 255;
                ptr[i][1] = 255;
            ...
            else if(nowPixel == 2)//左边界
            {
                ptr[i][0] = 0;
                ptr[i][1] = 0;
                ptr[i][2] = 255;
            }
            else if(nowPixel == 251)//右边界
            {
                ptr[i][0] = 0;
                ptr[i][1] = 255;
                ptr[i][2] = 0;
            }
            else if(nowPixel == 3)//中线
            {
                ptr[i][0] = 255;
                ptr[i][1] = 0;
                ptr[i][2] = 0;
...
Mat Widget::GrayMap2Mat(uint8_t map[my_row][_my_col], QPoint point)
{  //MUTIPLE应该是图像放大倍数，CV_8UC3表示图像是3通道的
    Mat mat(my_row * MULTIPLE, _my_col * MULTIPLE, CV_8UC3);
    for (int j = 0; j < mat.rows; ++j)
    {  //MUTIPLE是放大倍数，/MUTIPLE当然是真实的坐标了，不过是CV坐标系的，转成代码的坐标系还需要坐标转换一下
        Vec3b *ptr = mat.ptr<Vec3b>(j);
        for (int i = 0; i < mat.cols; ++i)
...
void Widget::imgGrayEvent(uint8_t img[my_row][_my_col], QLabel *label, uint8_t multiple)
{
    ...
    QPoint showPoint = adjustPixelInfo(relativePoint, my_row, my_col, multiple);
...
void Widget:: mapEvent(uint8_t img[my_row][my_col], QLabel *label, uint8_t multiple)
{
    QPoint relativePoint =
        label->mapFromGlobal(QPoint(cursor().pos().x(), cursor().pos().y()));
    int x = relativePoint.x() / multiple;
    int y = my_row  - relativePoint.y() / multiple;  //x, y 是allmap的坐标系
    mapPoint.rx() = relativePoint.x() / multiple;
    mapPoint.ry() = relativePoint.y() / multiple;  //mapPoint是CV的坐标系
    presspoint = mapPoint;
...
//    ui->rightmap_lable->installEventFilter(this);
    ui->imgGray_lable->installEventFilter(this);
     ui->Erzhihua_lable->installEventFilter(this);
}
...
bool Widget::eventFilter(QObject *object, QEvent *event)
{
        ...
        if (object == ui->imgGray_lable)
        {
            ...
            {
                imgGrayEvent(imgGray, ui->imgGray_lable, MULTIPLE);

                return true;
            }
        }
        else if (object == ui->Erzhihua_lable)
        {
            if (event->type() == QEvent::MouseButtonPress)
            {
                mapEvent(Exiacn,ui->Erzhihua_lable, MULTIPLE);
                return true;
            }
        }
...
void Widget::on_readData_pushButton_clicked()
{
    ...
    if(dataName != nullptr)
    {
        ...
        else
        {
            ...
            {
                int32 numSegments = (dataSize + SerialBuffer_AllNum - 1) / SerialBuffer_AllNum;  // 计算数据段的数量
                for (int32 i = 0; i < numSegments; i++)
                {
                    int32 startPos = i * SerialBuffer_AllNum;  // 数据段的起始位置
                    int32 segmentSize = qMin(SerialBuffer_AllNum, dataSize - startPos);  // 数据段的实际长度
                ...
                dataNum = dataNum_tips / SerialBuffer_AllNum;
            }
               /**********对滑动条的设置************/
...
void Widget::display_udp_data(QByteArray datatemp)
{
    ...
    if(ui->udp_connect_pushButton->text() == "中断连接" && udp_recv->connect_flag == true)
    {
        ...
        recv_wifiNum = recv_wifiNum + my_row * _my_col;
...
//        ui->dealImgTime_label->adjustSize();
        ...
        {
            Mat mat = GrayMap2Mat(imgGray, presspoint);
            showMat(mat, ui->imgGray_lable);
...
void Widget::on_udp_connect_pushButton_clicked()
{
    if(ui->udp_connect_pushButton->text() == "连接")
    {
        ...
        if( udp_recv->connect_flag)
        {
            ...
            ui->Wait_connect_label->setStyleSheet(
                        "background-color:rgb(100, 255, 100);font-family: Microsoft YaHei;font-size:20px;font-family: Microsoft YaHei;color:black");  //255 105 180
        ...
        else
        {
            ...
            ui->Wait_connect_label->setStyleSheet(
                        "background-color:rgb(255, 100, 100);font-family: Microsoft YaHei;font-size:20px;font-family: Microsoft YaHei;color:black");  //255 105 180
            ui->Wait_connect_label->adjustSize();
        }
    }
    else if(ui->udp_connect_pushButton->text() == "中断连接")
    {
        ...
        if(ui->WriteVideo_pushButton->text() == "停止||")
        {
...
void Widget::on_WriteVideo_pushButton_clicked()
{
    ...
    if(ui->Hostcomputer_Mode_tabWidget->currentIndex() == 1 )
    {
        ...
        {
                QMessageBox::information(this,"警告",QString("\n 未连接到任何客户端！\n"));
        }
    }
...
void Widget::on_wallpaper_pushbutton_clicked()
{
    ...
    {
        Wallpaper_Choice = 0;
        ui->label_9->setStyleSheet("QLabel#label_9 { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_10->setStyleSheet("QLabel#label_10 { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_11->setStyleSheet("QLabel#label_11 { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_12->setStyleSheet("QLabel#label_12 { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_13->setStyleSheet("QLabel#label_13 { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_14->setStyleSheet("QLabel#label_14 { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_16->setStyleSheet("QLabel#label_16 { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_38->setStyleSheet("QLabel#label_38 { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->SDDriveLetter_label->setStyleSheet("QLabel#SDDriveLetter_label { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Frame_rate_label->setStyleSheet("QLabel#Frame_rate_label { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->UTF8_WifiShow_checkBox->setStyleSheet("QCheckBox#UTF8_WifiShow_checkBox { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Hex_WifiShow_checkBox->setStyleSheet("QCheckBox#Hex_WifiShow_checkBox { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Hex_SerialShow_checkBox->setStyleSheet("QCheckBox#Hex_SerialShow_checkBox { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->UTF8_SerialShow_checkBox->setStyleSheet("QCheckBox#UTF8_SerialShow_checkBox { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Serial_ShowTime_checkBox->setStyleSheet("QCheckBox#Serial_ShowTime_checkBox { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Wifi_ShowTime_checkBox->setStyleSheet("QCheckBox#Wifi_ShowTime_checkBox { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Open_Wifi_checkBox->setStyleSheet("QCheckBox#Open_Wifi_checkBox { color: rgb(255, 200, 100); font-family: Microsoft YaHei; font-size: 20px; }");
    ...
    {
        Wallpaper_Choice = 1;
        ui->label_9->setStyleSheet("QLabel#label_9 { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_10->setStyleSheet("QLabel#label_10 { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_11->setStyleSheet("QLabel#label_11 { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_12->setStyleSheet("QLabel#label_12 { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_13->setStyleSheet("QLabel#label_13 { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_14->setStyleSheet("QLabel#label_14 { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_16->setStyleSheet("QLabel#label_16 { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->label_38->setStyleSheet("QLabel#label_38 { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->SDDriveLetter_label->setStyleSheet("QLabel#SDDriveLetter_label { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Frame_rate_label->setStyleSheet("QLabel#Frame_rate_label { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->UTF8_WifiShow_checkBox->setStyleSheet("QCheckBox#UTF8_WifiShow_checkBox { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Hex_WifiShow_checkBox->setStyleSheet("QCheckBox#Hex_WifiShow_checkBox { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Hex_SerialShow_checkBox->setStyleSheet("QCheckBox#Hex_SerialShow_checkBox { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->UTF8_SerialShow_checkBox->setStyleSheet("QCheckBox#UTF8_SerialShow_checkBox { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Serial_ShowTime_checkBox->setStyleSheet("QCheckBox#Serial_ShowTime_checkBox { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Wifi_ShowTime_checkBox->setStyleSheet("QCheckBox#Wifi_ShowTime_checkBox { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
        ui->Open_Wifi_checkBox->setStyleSheet("QCheckBox#Open_Wifi_checkBox { color: rgb(0, 0, 0); font-family: Microsoft YaHei; font-size: 20px; }");
